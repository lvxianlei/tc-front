...
import { materialColumns } from "./enquiryCompare.json"
...
export default function Overview(): JSX.Element {
    ...
    const { loading, data } = useRequest<{ [key: string]: any }>(() => new Promise(async (resole, reject) => {
        try {
            const result: { [key: string]: any } = await RequestUtil.get(`/tower-supply/auxiliaryComparisonPrice/${params.id}`)
    ...
    const { loading: finishPriceLoading, run: finishPriceRun } = useRequest<{ [key: string]: any }>((data: any) => new Promise(async (resole, reject) => {
        try {
            const result: { [key: string]: any } = await RequestUtil.post(`/tower-supply/auxiliaryComparisonPrice/completeComparisonPrice`, { ...data, id: params.id })
            resole(result)
        } catch (error) {
            reject(error)
        }
    ...
    const { run: deleteRun } = useRequest<{ [key: string]: any }>((id: string) => new Promise(async (resole, reject) => {
        try {
            const result: { [key: string]: any } = await RequestUtil.delete(`/tower-supply/auxiliaryInquiryQuotation?id=${id}`)
            resole(result)
        } catch (error) {
            reject(error)
        }
    ...
    return <Spin spinning={loading}>
        ...
        <DetailContent title={[
            ...
            <Button
                ...
                // disabled={data?.comparisonStatus !== 1}
                onClick={async () => {
                    if([0,'0',2,'2',3,'3',4,'4'].includes(params?.approvalStatus)){
