...
import { batchSupplier } from "./enquiryCompare.json"
...
export default forwardRef(function ({ id, type, materialLists, comparisonPriceId }: AddPriceProps, ref): JSX.Element {
    ...
    const { loading: suplierLoading, data: suplier } = useRequest<{ [key: string]: any }>(() => new Promise(async (resole, reject) => {
        try {
            const result: { [key: string]: any } = await RequestUtil.get(`/tower-supply/auxiliaryComparisonPrice/getSuppliersById/${comparisonPriceId}`)
    ...
    const { run: saveRun } = useRequest<{ [key: string]: any }>((data: any) => new Promise(async (resole, reject) => {
        try {
            const result: { [key: string]: any } = await RequestUtil[type === "batch_new" ? "post" : "put"](`/tower-supply/auxiliaryInquiryQuotation`, type === "batch_new" ? data : ({ id, ...data }))
            resole(result)
        } catch (error) {
            reject(error)
        }
