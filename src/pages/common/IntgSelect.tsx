...
export default function IntgSelect({ onChange, width, value = { first: "", second: "" } }: IntgSelectProps): JSX.Element {
    ...
    return <div style={{ width: width || "100%" }}>
        <Select style={{ width: "50%" }} onChange={handleFirstChange} value={deptId} >{deptData?.map((item: any) => <Select.Option value={item.id} key={item.id}>{item.name}</Select.Option>)}</Select>
        <Select style={{ width: "50%" }} onChange={handleChange} value={userId} disabled={!deptId}>{userData?.records?.map((item: any) => <Select.Option value={item.id} key={item.id}>{item.name}</Select.Option>)}</Select>
    </div>
}