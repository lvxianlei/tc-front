...
export default function Edit() {
    ...
    const { loading: creteLoading, run: createRun } = useRequest<{ [key: string]: any }>((saveData) => new Promise(async (resole, reject) => {
        try {
            const result: { [key: string]: any } = await RequestUtil.post(`/tower-market/CollectionNot`, saveData)
            resole(result)
        } catch (error) {
            reject(error)
        }
    ...
    const { loading: saveLoading, run: saveRun } = useRequest<{ [key: string]: any }>((saveData) => new Promise(async (resole, reject) => {
        try {
            const result: { [key: string]: any } = await RequestUtil.put(`/tower-market/CollectionNot/updatePayment`, saveData)
            resole(result)
        } catch (error) {
            reject(error)
        }
