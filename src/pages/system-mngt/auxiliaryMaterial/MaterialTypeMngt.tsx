...
class MaterialTypeMngt extends AbstractMngtComponent<IMaterialTypeTypeMngtWithRouteProps, IMaterialTypeTypeMngtState> {
    ...
    public onFinish = async (values: any) => {
        if (this.state.type === 0 || this.state.type === 2) {
            RequestUtil.post('/tower-system/materialCategory/auxiliary', {
                ...values,
                code: this.state.type === 0 ? values.code : this.state.defaultData.firstCode + values.code,
                parentId: this.state.type === 0 ? '0' : this.state.defaultData.parentId
            }).then(res => {
                this.setState({
                    visible: false,
                    defaultData: {}
            ...
            RequestUtil.put('/tower-system/materialCategory/auxiliary', {
                ...
                return {
                    ...
                    children: item?.children?.map((items: IMaterialType) => {
                        return {
                            ...
                            sonName: items.name,
                            name: '',
                            children: items.children && items.children?.length > 0 ? items.children?.map((item: any) => ({
                                ...item,
                                threeName: item.name,
                                name: '',
                            })) : undefined
