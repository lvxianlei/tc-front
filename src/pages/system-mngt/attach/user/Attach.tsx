...
export default forwardRef(function ({
    ...
    return <>
        ...
        {isTable && title && <DetailTitle
            ...
            {...edit ? {
                operation: [
                    <Space direction="horizontal" key="attach">
                        <Upload
                            key="sub"
                            name="file"
                            multiple={multiple}
        ...
        {isTable && <div style={{ border: "1px solid #eee", margin: "0px 0px 24px 0px", ...props?.style }}>
            <Row style={{ backgroundColor: "#fafafa", padding: 8, }}>
                ...
                <Col span={2}>大小</Col>
                <Col span={2}>上传时间</Col>
                <Col span={4} style={{ paddingLeft: 16, boxSizing: "border-box" }}>操作</Col>
            </Row>
            ...
            {isBatchDel ?
                <Checkbox.Group style={{ width: '100%' }} value={checkedList} onChange={onChange}>
                    {attchs.map((item, index: number) => <Spin key={item.uid} spinning={item.loading} size="small">
                        <Row style={{ backgroundColor: index % 2 === 0 ? "#fff" : "#f8f8f8", width: "100%" }} >
                            <Col span={2}>
                            ...
                            <Col span={2}>大小</Col>
                            <Col span={2}>上传时间</Col>
                            <Col span={4} style={{ padding: "0px 8px" }}>{operationRender(item)}</Col>
                        </Row>
                    </Spin>)}
                </Checkbox.Group>
                :
                <>{
                    attchs.map((item, index: number) => <Spin key={item.uid} spinning={item.loading} size="small">
                        <Row style={{ backgroundColor: index % 2 === 0 ? "#fff" : "#f8f8f8" }} >
                            ...
                            <Col span={2} style={{
                                padding: "8px 0px",
                            }}>{(Number(item.fileSize)/1024).toFixed(0)}KB</Col>
                            <Col span={2} style={{
                                padding: "8px 0px",
                            }}>{moment().format("YYYY-MM-DD")}</Col>
                            <Col span={4} style={{ padding: "0px 8px" }}>{operationRender(item)}</Col>
                        </Row>
                    </Spin>)
                }
