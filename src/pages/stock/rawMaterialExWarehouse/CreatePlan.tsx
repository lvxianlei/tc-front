                    ...
                    columns={[
                        ...
                        ...material.map((item: any) => {
                            if (["receiveBatchNumber"].includes(item.dataIndex)&&type===2) {
                                return ({
                                    ...item,
                                    width: 160,
                                    render: (value: number, records: any, key: number) => <Input defaultValue={value || undefined} onBlur={(e: any) => handleBatchChange(e.target.value, records.id)} key={key} maxLength={30} disabled={records?.outStockItemStatus!==0}/>
                                })
                            }
                            if (["num"].includes(item.dataIndex)&&type===0) {
                                return ({
                                    ...item,
                                    render: (value: number, records: any, key: number) => <InputNumber min={1} value={value || undefined} onChange={(value: number) => handleNumChange(value, records.id)} key={key}  disabled={records?.outStockItemStatus!==0}/>
                                })
                            }
                            ...
                            if (["length"].includes(item.dataIndex)&&type===2) {
                                return ({
                                    ...item,
                                    render: (value: number, records: any, key: number) => <InputNumber min={1} value={value || undefined} onChange={(value: number) => handleLengthChange(value, records.id)} key={key} disabled={records?.outStockItemStatus!==0}/>
                                })
                            }
                            if (["width"].includes(item.dataIndex)&&type===2) {
                                return ({
                                    ...item,
                                    render: (value: number, records: any, key: number) => <InputNumber min={1} value={value || undefined} onChange={(value: number) => handleWidthChange(value, records.id)} key={key} disabled={records?.outStockItemStatus!==0}/>
                                })
                            }
                            if (["reservoirName"].includes(item.dataIndex)&&type===2) {
                                return ({
                                    ...
                                    render: (value: string, records: any, key: number) => <Select
                                                ...
                                                disabled={records?.outStockItemStatus!==0}
                                            >
                                                {
                        ...
                        }),
                        {
                            ...
                            render: (_: any, records: any) => <>
                                ...
                                <Button type="link" disabled={records.source === 1||records?.outStockItemStatus!==0} onClick={() => handleRemove(records.id)}>移除</Button>
                            </>
                        }]}
                    pagination={false}
