    ...
    const paragraphColumns = [
        ...
        },
        {
            ...
            render: (_: any, record: Record<string, any>, index: number): React.ReactNode => (<Button type='link' onClick={() => {
                RequestUtil.post(`/tower-science/drawProductStructure/reuse?productCategoryId=${params?.id}&productSegmentGroupId=${params.productSegmentId}&productSegmentId=${paragraph || ""}&passivityProductSegment=${record.id}`).then(() => {
