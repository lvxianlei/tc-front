            ...
            <Page
                ...
                columns={[...tableColumns as any, {
                    ...
                    render: (_: undefined, record: any): React.ReactNode => (
                        <Space direction="horizontal" size="small" className={styles.operationBtn}>
                            <Button onClick={() => { history.push(`/workMngt/pickList/pickTowerMessage/${params.id}/${params.status}/${params.materialLeader}/pick/${record.id}`) }} type='link' disabled={!(user && user.length > 0 && user.map((item: any) => { return item.userId }).concat([record?.materialLeader]).indexOf(AuthUtil.getUserInfo().userId) > -1) || params.status === '1'}>提料</Button>
                            <Button onClick={() => { history.push(`/workMngt/pickList/pickTowerMessage/${params.id}/${params.status}/${params.materialLeader}/check/${record.id}/${record.materialLeader}`) }} type='link' disabled={record.status !== 2 || !(materialCheckLeaders.length > 0 && materialCheckLeaders.map((item: any) => { return item.userId }).concat([record?.materialCheckLeader]).indexOf(AuthUtil.getUserInfo().userId) > -1)}>校核</Button>
                ...
                extraOperation={
                    <Space>
                        ...
                        {(user && user.length > 0 && user.map((item: any) => { return item.userId }).concat([params?.materialLeader]).indexOf(AuthUtil.getUserInfo().userId) > -1) ?
                            ...
                            : null}
                        {(materialCheckLeaders.length > 0 && materialCheckLeaders.map((item: any) => { return item.userId }).concat([params.materialLeader]).indexOf(AuthUtil.getUserInfo().userId) > -1) ? <Popconfirm
                            title="确认提交?"
                            onConfirm={async () => {
                        ...
                        {(params.status === '1' || params.status === '2') && params.materialLeader === AuthUtil.getUserInfo().userId ? <TowerPickAssign title="塔型提料指派" id={params.id} update={onRefresh} path={pathLink} /> : null}
