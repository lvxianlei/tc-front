        ...
        },
        {
            ...
            render: (_: undefined, record: any): React.ReactNode => (
                <Space direction="horizontal" size="small" className={styles.operationBtn}>
                    <Button type='link' onClick={() => { history.push(`/workMngt/confirmList/confirmMessage/${record.id}`) }} disabled={AuthUtil.getUserInfo().user_id() !== record.confirmId}>确认信息</Button>
                    <Button type='link' onClick={() => { history.push(`/workMngt/confirmList/confirmDetail/${record.id}/${record.status}`) }} disabled={record.status < 3 || AuthUtil.getUserInfo().user_id() !== record.confirmId}>确认明细</Button>
                </Space>
            )
        }
    ];
