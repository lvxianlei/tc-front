...
export default function ContractMngt(): JSX.Element {
    ...
    const editRef = useRef<{ 
        ...
        resetFields: () => void,
        onSubmitCancel: () => void 
    }>({ onSubmit: () => { }, onSubmitApproval: () => { }, resetFields: () => { }, onSubmitCancel: () => {} })
    ...
    const handleEditModalOk = (type:'save'|'approvalSave'|'cancel') => new Promise(async (resove, reject) => {
        try {
            ...
            type==='cancel'&&setCancelLoading(true)
            type==='cancel'&&await editRef.current.onSubmitCancel()
            type==='cancel'&&setCancelLoading(false)
            // message.success("保存成功...")
            resove(true)
            type==='save'&&history.go(0)
            type==='approvalSave'&&history.go(0)
            type==='cancel'&&history.go(0)
        } catch (error) {
            type==='save'&&setSaveLoading(false)
            type==='approvalSave'&&setSubmitLoading(false)
            type==='cancel'&&setCancelLoading(false)
            ...
            <Modal
                ...
                footer={oprationType === 'edit'?<Space>
                    ...
                    <Button type='primary' onClick={()=>{
                        handleEditModalOk('save')
                    }} loading={saveLoading}>保存</Button>
                    <Button type='primary' onClick={()=>{
                        handleEditModalOk('approvalSave')
                    }} loading={submitLoading}>保存并提交审批</Button>
                    <Button type='primary' onClick={()=>{
                        handleEditModalOk('cancel')
                    }} loading={cancelLoading}>撤销审批</Button>
                </Space>: <Space>
                    <Button onClick={() => {
                        editRef.current?.resetFields()
                        setDetailId("")
